...
class PointCloudCluster(Node):
    ...
    def pointcloud_callback(self, msg):
        ...
        # Publish cluster centroids as markers
    ...
    def publish_cluster_markers(self):
        """ Publishes cluster centroids as visualization markers """
        ...
        for i, centroid in enumerate(self.cluster_centroids):
            ...
            marker.id = i
            marker.type = Marker.SPHERE  # Can also use Marker.CUBE
            marker.action = Marker.ADD
            marker.pose.position.x = float(centroid[0])
            marker.pose.position.y = float(centroid[1])
            marker.pose.position.z = 1.0 #float(centroid[2])
            marker.pose.orientation.w = 1.0  # No rotation
            
            marker.scale.x = 1.0  # Adjust size
            marker.scale.y = 1.0
            marker.scale.z = 1.0
            ...
            marker.lifetime.sec = 2000  # Markers disappear after 2 seconds
            marker_array.markers.append(marker)

        self.publisher.publish(marker_array)
        self.get_logger().info(f'Published {len(self.cluster_centroids)} cluster centroids as markers')
